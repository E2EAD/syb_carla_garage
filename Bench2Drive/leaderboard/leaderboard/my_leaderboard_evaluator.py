...
class LeaderboardEvaluator(object):
    ...
    def _setup_simulation(self, args):
        ...
        cmd1 = f"{os.path.join(self.carla_path, 'CarlaUE4.sh')} -RenderOffScreen -nosound -carla-rpc-port={args.port} -graphicsadapter={args.gpu_rank}"
        self.server = subprocess.Popen(cmd1, shell=True, preexec_fn=os.setsid)
        print(f"Started CARLA server on port {args.port}", flush=True)
        atexit.register(os.killpg, self.server.pid, signal.SIGKILL)

        # Wait for CARLA server to be ready
        print("Waiting for CARLA server to become available...", flush=True)
        while True:
            try:
                client = carla.Client(args.host, args.port)
                client.set_timeout(10.0)
                # This will raise if no server is available
                client.get_server_version()
                print("CARLA server is ready!", flush=True)
                break
            except Exception as e:
                print(f"CARLA server not available yet: {e}", flush=True)
                time.sleep(2)

        # Now proceed with settings
        ...
        # Traffic Manager setup with retry
        attempts = 0
        while attempts < 40:
            try:
                args.traffic_manager_port = find_free_port(args.traffic_manager_port)
                ...
                traffic_manager.set_synchronous_mode(True)
                traffic_manager.set_hybrid_physics_mode(True)
                print(f"Traffic Manager initialized on port {args.traffic_manager_port}", flush=True)
                break
            except Exception as e:
                print(f"TM init failed (attempt {attempts}): {e}", flush=True)
                attempts += 1
                time.sleep(2)
        else:
            raise RuntimeError("Could not connect to Traffic Manager after multiple attempts.")

        return client, args.timeout, traffic_manager
