...
>>>>>>> 292b63d6ceceb7e250022de6871d308bc00b4f72
    ...
    for i in range(self.model_count):
      if self.config.backbone in ('transFuser', 'aim', 'bev_encoder'):
        pred_wp, \
        pred_target_speed, \
        pred_trajectories, pred_traj_probs, \
        pred_semantic, \
        pred_bev_semantic, \
        pred_depth, \
        pred_bb_features,\
        ...
        best_anchor_indices = torch.argmax(pred_traj_probs, dim=0)  # (batch_size,)
...
<<<<<<< HEAD
      ...
      # 使用集成平均的速度预测
      if self.config.use_controller_input_prediction:
          # print_data_info(pred_target_speeds[0])
          pred_target_speed_ensemble = torch.stack(pred_target_speeds, dim=0).mean(dim=0)
          viz_prob_target_speed = F.softmax(pred_target_speed_ensemble, dim=0)
          # print_data_info(viz_prob_target_speed)
      # else:
      #     viz_prob_target_speed = self.pred_target_speed_scalar  # 或其他合适的变量
      
      # 使用集成平均的检查点
      if self.config.use_controller_input_prediction:
          # print_data_info(pred_checkpoints[0])
          viz_checkpoint = torch.stack(pred_checkpoints, dim=0).mean(dim=0)
          # print_data_info(viz_checkpoint)
