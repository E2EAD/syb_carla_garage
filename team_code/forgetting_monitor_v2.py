...
class ForgettingMonitor:
  ...
  def check_and_act(self, current_model, data_loader, device, global_step):
    ...
    return self._decide_action(kl_values, device)

  def check_batch_and_act(self, current_model, data_batch, device, global_step):
    monitor_every = int(getattr(self.config, 'monitor_kl_every', 100))
    if monitor_every > 0 and self.last_check_step >= 0 and global_step - self.last_check_step < monitor_every:
      return 'continue', {}
    self.last_check_step = global_step
    ...
    return self._decide_action([self.estimate_kl(current_model, data_batch, device)], device)

  def _decide_action(self, kl_values, device):
    avg_kl = float(np.mean(kl_values))
    if dist.is_available() and dist.is_initialized():
      kl_tensor = torch.tensor(avg_kl, dtype=torch.float32, device=device)
      dist.all_reduce(kl_tensor, op=dist.ReduceOp.SUM)
      avg_kl = float((kl_tensor / dist.get_world_size()).item())
    ...
    if avg_kl > self.kl_threshold:
      if len(self.kl_history) >= self.patience and all(k > self.kl_threshold
        ...
        print(f'Early stopping. Forgetting KL: {metrics["forgetting_kl"]}', flush=True)
        return 'early_stop', metrics
      print(f'Reducing learning rate. Forgetting KL: {metrics["forgetting_kl"]}', flush=True)
      return 'reduce_lr', metrics

    print(f'Continue. Forgetting KL: {metrics["forgetting_kl"]}', flush=True)
