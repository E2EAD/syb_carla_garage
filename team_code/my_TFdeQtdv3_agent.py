  ...
  def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    for i in range(self.model_count):
      if self.config.backbone in ('transFuser', 'aim', 'bev_encoder'):
        ...
        # After computing pred_speed_probs
        best_anchor_indices = torch.argmax(pred_speed_probs, dim=0)  # (batch_size,)
        batch_indices = torch.arange(pred_speed_probs.size(1), device=pred_speeds.device)
        # Select best speed for each batch element
        pred_target_speed = pred_speeds[best_anchor_indices, batch_indices]  # (batch_size, 10, 2)
        # print_data_info(pred_checkpoint)  # torch.Size([1, 10, 2])
