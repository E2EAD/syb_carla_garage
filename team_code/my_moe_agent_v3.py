...
from my_model_moe_v3 import LidarCenterNet
  ...
  def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    for i in range(self.model_count):
      if self.config.backbone in ('transFuser', 'aim', 'bev_encoder'):
        pred_wp, \
        pred_target_speed, \
        pred_trajectories, pred_traj_probs, pred_speeds, \
        pred_semantic, \
        pred_bev_semantic, \
        pred_depth, \
        pred_bb_features,\
        ...
        pred_speed = pred_speeds[best_anchor_indices, batch_indices]
      ...
      if self.config.use_controller_input_prediction:  # True
        pred_target_speeds.append(F.softmax(pred_speed[0], dim=0))
