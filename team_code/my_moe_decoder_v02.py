...
class PlanningTrajectoryDecoder(nn.Module):
    ...
    - selected_trajectories: (batch_size, self.cfg.expert_out_dim) - [x1,y1,x2,y2,...,x6,y6] coordinates
    - all_trajectories: (num_anchors, batch_size, self.cfg.expert_out_dim) - All predicted trajectories
    ...
    def __init__(self, cfg: GlobalConfig, num_anchors=53):
        ...
        # Create diverse trajectory anchors (num_anchors, self.cfg.expert_out_dim)
        ...
        # self.offset_head = nn.Linear(cfg.tf_de_dim, self.cfg.expert_out_dim)
        self.offset_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(cfg.tf_de_dim//4, self.cfg.expert_out_dim)
        )
    ...
    def _create_anchors(self, anchor_path=None):
        ...
        """
        if anchor_path == None:
            print('no anchor path, mamba out.')
            return
        else:
            ...
            mu_list = [entry['mu'][:self.cfg.expert_out_dim] for entry in data]
    ...
    def forward(self, encoder_out):
        ...
        Returns:
            pred_trajectories: (num_anchors, batch_size, self.cfg.expert_out_dim) - Predicted trajectories
        ...
        pred_trajectories = (anchors_expanded + offsets)[:,:,:20]  # (num_anchors, batch_size, 20)
        pred_speeds = (anchors_expanded + offsets)[:,:,20:]
