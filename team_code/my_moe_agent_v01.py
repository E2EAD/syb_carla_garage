...
class SensorAgent(autonomous_agent.AutonomousAgent):
  ...
  def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    for i in range(self.model_count):
      if self.config.backbone in ('transFuser', 'aim', 'bev_encoder'):
        pred_wp, \
        pred_target_speed, \
        pred_trajectories, pred_traj_probs, task_latent_mu, task_latent_log_var, reconstructed_features,  \
        pred_semantic, \
        pred_bev_semantic, \
        pred_depth, \
        pred_bb_features,\
        ...
        # print(f'pred_traj_probs = {pred_traj_probs}')
        # print(f'best_anchor_indices = {best_anchor_indices}')
        ...
        # print_data_info(kl_div)
        # print(f'kl_div= {kl_div}')
        
        best_anchor_indices = torch.argmin(kl_div, dim=-1)  # (batch_size,)
        # print_data_info(best_anchor_indices)
        # print(f'best_anchor_indices= {best_anchor_indices}')
  ...
  def compute_kl_div(self, mu, log_var, anchor_mu, anchor_var):
    """
