  ...
  def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
    ...
    for file in os.listdir(self.config_path):
      if file.endswith('.pth') and file.startswith('model'):
        ...
        net.load_state_dict(state_dict, strict=True)  # !!! should be True
  ...
  def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    for i in range(self.model_count):
      if self.config.backbone in ('transFuser', 'aim', 'bev_encoder'):
        ...
        # 同步并记录时间
        # print_data_info(pred_target_speed)  # Shape: torch.Size([1, 8])
        torch.cuda.synchronize() if torch.cuda.is_available() else None
        inference_time = time.time() - start_time
        self.batch_infer_time.append(inference_time)
        ...
        self.peak_gpu_memo.append(peak_memory)

        self.batch_cnt = self.batch_cnt + 1

        if self.batch_cnt > 0 and self.batch_cnt % 10 == 0:
            np_batch_infer_time = np.array(self.batch_infer_time)
            np_peak_gpu_memo = np.array(self.peak_gpu_memo)
            print(f'===== Mean infer FPS = {round(float(1 / np.mean(np_batch_infer_time)), 2)} Hz =====')
            print(f'===== Mean infer time = {round(float(1000 * np.mean(np_batch_infer_time)), 2)} ms =====')
            print(f'===== Mean GPU memory allocated peak = {round(float(np.mean(np_peak_gpu_memo)), 2)} mb =====')
