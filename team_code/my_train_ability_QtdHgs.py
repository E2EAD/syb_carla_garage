...
from my_model_QtdHgs import LidarCenterNet
...
def main():
  ...
  parser.add_argument('--hgs_ref_file',
                      type=str,
                      default=str(getattr(config, 'hgs_ref_file', config.a3d_ref_file)),
                      help='Reference model directory for HGS (config.json + model_*.pth).')
...
#     model.load_state_dict(torch.load(args.load_file, map_location=device), strict=False)
  ...
  teacher_model = None
  if bool(config.hgs_enable):
    hgs_ref_dir = getattr(config, 'hgs_ref_file', None)
    if hgs_ref_dir is None or str(hgs_ref_dir).strip() == '':
      hgs_ref_dir = config.a3d_ref_file
    if hgs_ref_dir is None or str(hgs_ref_dir).strip() == '':
      raise ValueError('HGS is enabled but neither --hgs_ref_file nor --a3d_ref_file is provided.')
    teacher_model, teacher_ckpt = load_teacher_from_dir(hgs_ref_dir, device)
    if rank == 0:
      print(f'Loading frozen HGS reference dir: {hgs_ref_dir}', flush=True)
      print(f'Reference checkpoint selected: {teacher_ckpt}', flush=True)
...
class Engine(object):
  ...
  def load_data_compute_loss(self, data, validation=False):
    ...
    # HGS uses a frozen reference model for advantage-based gradient surgery only.
    if bool(self.hgs_enable) and (self.teacher_model is not None) and (not validation) and (not self.config.use_plant):
      with torch.no_grad():
      ...
      # No A3D KD loss is added in QTD+HGS mode.
