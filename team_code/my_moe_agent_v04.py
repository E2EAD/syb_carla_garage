...
from my_model_moe_v04 import LidarCenterNet
  ...
  def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    for i in range(self.model_count):
      if self.config.backbone in ('transFuser', 'aim', 'bev_encoder'):
        pred_wp, \
        pred_target_speed, \
        pred_trajectories, pred_traj_probs, task_latent_mu, task_latent_log_var, reconstructed_features, task_anchor_loss, task_alignment_metrics,  \
        pred_semantic, \
        pred_bev_semantic, \
        pred_depth, \
        pred_bb_features,\
