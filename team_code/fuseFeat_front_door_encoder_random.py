...
class FuseFeatFrontDoorEncoder(nn.Module):
    """
    fuse feat前门编码器 - 处理fuse feat特征和随机初始化query token的交互
    fuse feat特征: (batch_size, 11, 256) 11个token序列
    query tokens: (num_queries, 11*256) 11*256维随机初始化可学习向量
    """
    def __init__(self, config):
        ...
        self.num_queries = getattr(config, 'random_query_fuse_feat_num_queries', getattr(config, 'fuse_feat_num_queries', 99))
        self.query_fuseFeat_tokens = nn.Parameter(torch.empty(self.num_queries, 256 * 11))
        ...
        nn.init.xavier_uniform_(self.query_fuseFeat_tokens)
    ...
    def forward(self, fuseFeat_feats, fuseFeat_mask=None):
        ...
        num_anchors, anchor_dim = self.query_fuseFeat_tokens.shape
        ...
        # 4. 投影随机初始化query token
        global_feats = self.anchor_proj(self.query_fuseFeat_tokens)  # [num_anchors, 256]
