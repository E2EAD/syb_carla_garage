...
def _compute_success_score(pred_trajectories, pred_traj_probs, speed_logits, checkpoint, target_speed, config):
  top1_traj = _select_top1_trajectory(pred_trajectories, pred_traj_probs)
  if top1_traj is None or speed_logits is None:
    return None
  with torch.no_grad():
    ...
    speed_acc = _compute_speed_acc_from_logits(speed_logits, target_speed)
    if speed_acc is None:
      return None
    score = config.a3d_w_traj * traj_score + config.a3d_w_speed * speed_acc
  return score
  ...
  def load_data_compute_loss(self, data, validation=False):
    ...
    if bool(self.config.use_a3d) and (self.teacher_model is not None) and (not validation) and (not self.config.use_plant):
      ...
      if teacher_tensors is not None and student_tensors is not None:
        s_ref = _compute_success_score(
            teacher_tensors.get('pred_trajectories'),
            teacher_tensors.get('traj_probs'),
            teacher_tensors.get('speed_logits'),
            checkpoint,
            target_speed,
            self.config,
        )
        s_active = _compute_success_score(
            student_tensors.get('pred_trajectories'),
            student_tensors.get('traj_probs'),
            student_tensors.get('speed_logits'),
            checkpoint,
            target_speed,
            self.config,
        )

        if s_ref is not None and s_active is not None:
          # Relative advantage: teacher better than student.
          a_rel = torch.mean(s_ref - s_active).detach()
          ref_acc = torch.mean(s_ref).detach()
          lambda_raw = torch.sigmoid(self.config.a3d_beta * a_rel).item()
          if float(ref_acc.item()) < float(self.config.a3d_tau):
            lambda_raw = 0.0
          self.a3d_lambda_ema = (self.config.a3d_lambda_ema * self.a3d_lambda_ema +
                                 (1.0 - self.config.a3d_lambda_ema) * lambda_raw)
          lambda_kd = min(self.config.a3d_lambda_max, self.a3d_lambda_ema)
          ...
          loss_traj_kd = _safe_kl(s_traj_logits, t_traj_logits, t_kd)
          ...
          offset_abs = torch.abs(s_traj - t_traj).sum(dim=(-1, -2))  # (B, K)
          loss_offset = torch.mean(torch.sum(offset_abs * t_traj_prob, dim=-1))
          ...
          losses['loss_a3d_total'] = torch.tensor(lambda_kd, device=self.device, dtype=loss_kd.dtype) * loss_kd
          losses['loss_a3d_traj_kd'] = torch.tensor(lambda_kd, device=self.device, dtype=loss_kd.dtype) * loss_traj_kd
          losses['loss_a3d_speed_kd'] = torch.tensor(lambda_kd, device=self.device, dtype=loss_kd.dtype) * loss_speed_kd
          losses['loss_a3d_offset_kd'] = torch.tensor(lambda_kd, device=self.device, dtype=loss_kd.dtype) * loss_offset
          losses['a3d_lambda'] = torch.tensor(lambda_kd, device=self.device, dtype=loss_kd.dtype)
          losses['a3d_advantage'] = a_rel
          losses['a3d_ref_score'] = ref_acc
          losses['a3d_active_score'] = torch.mean(s_active).detach()
