...
from mode_traj_decoder import MoDETrajectoryDecoder
...
class LidarCenterNet(nn.Module):
  ...
  def __init__(self, config):
    ...
    if self.config.use_controller_input_prediction or self.config.use_wp_gru:
      if self.config.transformer_decoder_join:
        ...
        #                                                             target_point_size=target_point_size)
          self.query_traj_decoder = MoDETrajectoryDecoder(self.config)
    ...
    if self.config.use_wp_gru or self.config.use_controller_input_prediction:  # False or 0/1
      ...
      if self.config.transformer_decoder_join:
        ...
        #   pred_checkpoint = self.checkpoint_decoder(aug_gru_features, target_point) # here we have pred checkpoints
          ...
          mode_aux_losses = getattr(self.query_traj_decoder, 'last_aux_losses', {})
          ...
          distill_tensors.update(mode_aux_losses)
  ...
  def compute_loss(self, pred_wp, pred_target_speed, pred_trajectories, pred_traj_probs, pred_semantic, pred_bev_semantic, pred_depth,
    ...
    if self.config.use_controller_input_prediction and pred_target_speed is not None:
      loss_target_speed = self.loss_speed(pred_target_speed, target_speed_label)
      loss.update({'loss_target_speed': loss_target_speed})

    if self.config.use_controller_input_prediction and pred_trajectories is not None and pred_traj_probs is not None:
      num_anchors = pred_traj_probs.size(0)
      batch_size = pred_traj_probs.size(1)
      with torch.no_grad():
        distances = torch.norm(pred_trajectories - checkpoint_label.unsqueeze(0), dim=-1).sum(dim=-1)
        distances = torch.clamp(distances, min=1e-6, max=100.0)
        neg_distances = -distances / float(getattr(self.config, 'temperature', 1.0))
        neg_distances = neg_distances - neg_distances.max(dim=0, keepdim=True)[0]
        soft_labels = F.softmax(neg_distances, dim=0).clamp(min=1e-8, max=1.0)

      kl_loss = F.kl_div(
          torch.log(pred_traj_probs + 1e-8),
          soft_labels,
          reduction='batchmean',
          log_target=False,
      )
      ...
      regression_losses = F.smooth_l1_loss(pred_trajectories, checkpoint_expanded, reduction='none')
      regression_losses = regression_losses.sum(dim=(-1, -2))
      ...
      _, best_indices = torch.min(distances, dim=0)
      best_pred_trajectories = pred_trajectories[best_indices, torch.arange(batch_size, device=best_indices.device)]
      ...
      mode_aux_losses = getattr(self.query_traj_decoder, 'last_aux_losses', {})
      for key, value in mode_aux_losses.items():
        loss[key] = value
