...
class TrajFrontDoorEncoder(nn.Module):
    """
    前门编码器 - 处理GRU特征和随机初始化query token的交互
    """
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        self.num_queries = getattr(config, 'random_query_traj_num_queries', getattr(config, 'traj_num_queries', 99))
    ...
    def forward(self, local_feats, local_mask=None):
        ...
        Args:
            ...
            query_traj_tokens: (num_queries, 20) - 随机初始化可学习query token
        ...
        num_anchors, anchor_dim = self.query_traj_tokens.shape
        
        # 1. 投影query token到特征空间 (num_queries, 20) -> (num_queries, 10, 256)
        global_feats = self.anchor_proj(self.query_traj_tokens)  # [num_queries, 10, 256]
