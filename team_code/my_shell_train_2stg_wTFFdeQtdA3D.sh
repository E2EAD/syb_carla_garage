...
torchrun --nnodes=1 --nproc_per_node=1 --max_restarts=1 --rdzv_id=251122 --rdzv_backend=c10d \
  team_code/my_train_ability_wTFFdeQtdA3D.py \
  --id syb_TFFdeQtdA3D_0-eb_2stg_demo \
  ...
  --freeze_backbone 0 --use_depth 0 --use_semantic 0 --detect_boxes 1 --use_bev_semantic 1 \
  --image_architecture regnety_032 --lidar_architecture regnety_032 \
  --load_file ${STUDENT_INIT_CKPT} \
  --use_a3d 1 \
  --a3d_ref_file ${TEACHER_DIR}
